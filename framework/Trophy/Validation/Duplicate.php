...
class Duplicate
{
    public function check($fields, $entries = 1, $hours = NULL)
    {
        ...
        foreach($fields as $field) {
            $db_fields .= $field . ", ";
            $form_fields .= $field . ' = ' . sprintf("'%s'", $this->_sanitise($_POST[$field])) . ' AND ';
        }
        ...
        $sql = sprintf("SELECT COUNT(entry_id) AS records FROM " . $this->_sanitise($config->db_table) . " WHERE %s;", $form_fields);
    ...
    // Repeated in Form
    private function _sanitise($input)
    {
